...
if __name__ == '__main__':
    ...
    parser.add_argument('--res_path', default='./case103', help='path to the config file')
    ...
    for file_name in os.listdir(args.res_path):
        ...
        config = input("No configuration file is found associated with the results, provided it manually:\n")
    ...
    for file_name in os.listdir(args.res_path):
        if "seed" + str(args.seed) in file_name and file_name.endswith('pickle') and file_name.startswith('reversekl'):
    ...
    if args.loss_plot:
        ...
        if args.save:
            plt.title('')
            plt.savefig('loss_case' + str(testcase_dict['case_index']) + '_seed' + str(args.seed) + '.png')
    ...
    for file_name in os.listdir(args.res_path):
        ...
        raise RuntimeError("The network model is not found")
            ...
            if args.save:
                if i != num_steps:
                    if args.middle_plot:
                        ...
                        plt.title('')
                        plt.savefig('learnt_proposal_' + str(i) + '_' + str(args.testcase) + '.png', bbox_inches='tight')
                else:
    ...
    if args.box_plot:
        ...
        if args.save:
            ...
            plt.title('')
            plt.savefig('box_' + str(args.testcase) + '.png', bbox_inches='tight')
